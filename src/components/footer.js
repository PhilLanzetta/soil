...
const Footer = ({ location }) => {
  ...
  const [amsterdamWeather, setAmsterdamWeather] = useState()
  const [email, setEmail] = useState("")
  const [inquireState, setInquireState] = useState({})
  const { popUp, setPopUp, pressPopUp, setPressPopUp } = useMyContext()
  ...
  if (nyWeather) {
    if (
      nyWeather.weather[0].icon === "01d" ||
      nyWeather.weather[0].icon === "02d"
    ) {
      nyWeatherIcon = Sunny
    } else if (
      nyWeather.weather[0].icon === "04d" ||
      nyWeather.weather[0].icon === "50d" ||
      nyWeather.weather[0].icon === "03d"
    ) {
      nyWeatherIcon = Cloudy
    } else if (
      nyWeather.weather[0].icon === "09d" ||
      nyWeather.weather[0].icon === "10d"
    ) {
      nyWeatherIcon = Showers
    } else if (nyWeather.weather[0].icon === "11d") {
      nyWeatherIcon = Thunderstorm
    } else {
      nyWeatherIcon = Night
    }
  }
  ...
  if (amsterdamWeather) {
    if (
      amsterdamWeather.weather[0].icon === "01d" ||
      amsterdamWeather.weather[0].icon === "02d"
    ) {
      amWeatherIcon = Sunny
    } else if (
      amsterdamWeather.weather[0].icon === "04d" ||
      amsterdamWeather.weather[0].icon === "50d" ||
      amsterdamWeather.weather[0].icon === "03d"
    ) {
      amWeatherIcon = Cloudy
    } else if (
      amsterdamWeather.weather[0].icon === "09d" ||
      amsterdamWeather.weather[0].icon === "10d"
    ) {
      amWeatherIcon = Showers
    } else if (amsterdamWeather.weather[0].icon === "11d") {
      amWeatherIcon = Thunderstorm
    } else {
      amWeatherIcon = Night
    }
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?id=5128581&units=imperial&appid=${API_KEY}`
      )
      const data = await response.json()
      setNyWeather(data)
    }
    ...
    <>
      <footer>
        ...
        >
          ...
          <div className={styles.footerGrouping}>
            <div className={styles.footerColumn}>
              <a href={`${process.env.GATSBY_SITE_URL}/about/#careers`}>
                Careers
              </a>
            ...
            <div className={styles.footerColumn}>
              ...
              <a
                href="https://www.linkedin.com/company/so-il"
                rel="noreferrer"
                target="_blank"
              >
                LinkedIn
